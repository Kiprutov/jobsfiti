...
export default function JobsPage() {
  ...
  const categories = [
    ...
    ...cat,
    count: jobs.filter(
      (j: FirestoreJob) => j.category === cat.id && (!role || j.role === role)
    ).length,
  }));
  ...
  const roles = [
    ...
    count: jobs.filter(
      (j: FirestoreJob) =>
        j.role === r.id && (!category || j.category === category)
    ).length,
  }));
            ...
            <div className="lg:col-span-1">
              <div className="bg-white border border-slate-200 rounded-md p-4 space-y-4">
                <div>
                  ...
                  <p className="text-slate-600 text-sm">{summaryText}</p>
                </div>
              ...
              {isLoading ? (
                ...
                <div className="space-y-4">
                  {filteredJobs.map((job) => (
                    <div
                      key={job.id}
                      className="bg-white border border-gray-200 rounded-lg p-6 transition-all duration-200 hover:border-blue-300 hover:shadow-sm"
                    >
                      <div className="flex flex-col space-y-4">
                        <div className="flex justify-between items-start">
                          ...
                          <span className="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded">
                            Experience:{" "}
                            {job.requirements?.experience?.years || "N/A"}
                          </span>
                        </div>
