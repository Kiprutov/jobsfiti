...
export default function JobsPage() {
  ...
  return (
    <div className={(showForm || editingJob) ? "relative h-[calc(100vh-7rem)]" : "space-y-6"}>
      {!showForm && !editingJob && (
      ...
      {/* Job Form Wizard */}
      {(showForm || editingJob) && (
        <div className="absolute inset-0 -mx-6 -my-6">
          <JobFormWizard
            ...
            {isLoading ? (
              ...
              <div className="space-y-4">
                {filteredJobs.map((job) => (
                  <div key={job.id} className="border rounded-lg p-4 hover:border-blue-500 transition-colors">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <h3 className="font-semibold text-lg">{job.title}</h3>
                        <p className="text-sm text-gray-500 mt-1">
                          {job.companySnapshot?.name || job.companyName} • {job.location}
                        </p>
                        ...
                        <div className="flex gap-2 mt-3 flex-wrap">
                          ...
                          <span className={`px-2 py-1 text-xs rounded-full ${job.status === 'open' ? 'bg-green-500 text-white' :
                            job.status === 'closed' ? 'bg-red-500 text-white' :
                              job.status === 'paused' ? 'bg-yellow-500 text-white' :
                        ...
                        <div className="flex gap-4 mt-3 text-xs text-gray-500">
                          ...
                          <span>Views: {job.audit?.views || 0}</span>
                          <span>Applications: {job.audit?.applications || 0}</span>
                        </div>
                      </div>
